...
bool RMLContext::is_colliding( const vertex& q ) const {
  ...
  robotstate.setJointGroupPositions( "hector_moveit_planner", q );
  robotstate.update();
  
  if( getPlanningScene()->isStateColliding( robotstate, "hector_moveit_planner", false ) )
    {
      return true;
    }
  else
...
RMLContext::vertex RMLContext::sample_nearby( const RMLContext::vertex& q ){
  ...
  double q_temp = 0;
  Eigen::Quaternionf quat(q[6], q[3], q[4], q[5]); 
  ...
  std::uniform_real_distribution<double> distribution_2(-0.1,0.1);
  ...
  while(true)
  {
    for(int i=0; i < q.size()-4; ++i)
    {
      ...
      if(i == 0 || i == 1 || i == 2)
      {
        while(q_temp > 8 || q_temp < -8 || (i==2 && q_temp < 0))
        {
          q_temp = q[i] + distribution(generator);
        }
      }
    ...
    euler[0] = euler[0] + distribution_2(generator);
    euler[1] = euler[1] + distribution_2(generator);
    euler[2] = euler[2] + distribution(generator);

    quat = Eigen::AngleAxisf(euler[0], Eigen::Vector3f::UnitX())* Eigen::AngleAxisf(euler[1], Eigen::Vector3f::UnitY())* Eigen::AngleAxisf(euler[2], Eigen::Vector3f::UnitZ());
...
bool RMLContext::solve( planning_interface::MotionPlanResponse &res ){
  ...
  moveit::core::RobotState robotstate( robotmodel );
  robotstate.setJointGroupPositions( "hector_moveit_planner", q_init );
